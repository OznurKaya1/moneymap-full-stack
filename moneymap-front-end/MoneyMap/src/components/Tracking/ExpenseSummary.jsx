      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: monthlyTotals.map(m =>
            m.month === selectedMonth ? "#378ADD" : "#B5D4F4"
          ),
          borderRadius: 6,
          borderSkipped: false,
        }],
  ...
  return (
    <div className="tracker-page">
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem" }}>
        ...
        <button className="tracker-btn tracker-btn-secondary" onClick={() => navigate(-1)}>
          ← Back to Expenses
        </button>
      </div>
      ...
      {filtered.length > 0 ? (
        <div className="tracker-card" style={{ marginBottom: "1.5rem" }}>
          <h2 style={sectionTitle}>
            {selectedMonth ? `${formatMonthLong(selectedMonth).toLowerCase()} — breakdown` : "all time — breakdown"}
          </h2>
      ...
      {filtered.length > 0 && (
        <div className="tracker-card">
          <h2 style={sectionTitle}>
            {selectedMonth ? `${formatMonthLong(selectedMonth).toLowerCase()} — all entries` : "all entries"}
          </h2>
